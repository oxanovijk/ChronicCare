...
export function OcrReviewWorkspace() {
  ...
  return (
    <div className="review-workspace">
      <section className="document-preview"><div className="preview-toolbar"><span>Dokumen privat sintetis</span><strong>Halaman 1 / 1</strong></div><article><span>KLINIK SEHAT BERSAMA · DEMO</span><h2>Ringkasan Kontrol Rutin</h2><p>Nama: Maya Pratama</p><p>Tanggal dokumen: 15 Juli 2026</p><hr /><p>Dokumen contoh untuk mencoba proses review.</p></article></section>
      <form className="extraction-panel" onSubmit={(event) => event.preventDefault()}><div className="review-title-row"><div><span className="section-kicker"><span aria-hidden="true" /> Screen 06</span><h1>Review hasil ekstraksi</h1></div><span className="review-status">Perlu review</span></div><p>Cocokkan hasilnya dengan dokumen asli. ChroniCare hanya menyalin informasi dan tidak memberikan penilaian medis.</p><label>Nama dokumen<input aria-label="Nama dokumen" value={title} onChange={(event) => setTitle(event.target.value)} /></label>{title !== initial ? <span className="edited-label">Diedit caregiver</span> : null}<label>Tanggal dokumen<input value="2026-07-15" readOnly /></label><label>Ringkasan netral<textarea defaultValue="Kontrol rutin tercatat. Tidak ada penilaian medis dari ChroniCare." /></label><div className="review-actions"><button className="primary-button" type="button" onClick={() => setStatus("confirmed")}>Konfirmasi ekstraksi</button><button className="secondary-button" type="button" onClick={() => setStatus("rejected")}>Tolak hasil</button></div></form>
    </div>
  );
}
