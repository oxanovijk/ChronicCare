...
export function CaregiverSosPanel() {
  ...
  return (
    <div className="sos-dashboard">
      ...
      <aside className="sos-controls"><span className="card-index">Pengaturan alert</span><h2>Suara bersifat opsional.</h2><label className="switch-row"><span><strong>Aktifkan suara</strong><small>Suara hanya aktif selama halaman ini terbuka dan perangkat mengizinkan.</small></span><input type="checkbox" checked={audio} onChange={(event) => setAudio(event.target.checked)} /></label><p className="connection-note">Alert visual tetap terlihat dengan atau tanpa suara. Saat koneksi kembali, status terbaru akan ditampilkan.</p><button className="text-button" type="button" onClick={() => setConflict(true)}>Lihat contoh saat sudah ditangani</button>{conflict ? <p className="conflict-note" role="status">Alert ini baru saja ditangani oleh anggota lain. Status telah diperbarui dengan aman.</p> : null}</aside>
    </div>
  );
}
